...
const dutyWarningAcknowledge = document.getElementById("dutyWarningAcknowledge");
...
function updateDutyGateState() {
  const acknowledged = dutyWarningAcknowledge.checked;
  punchInFields.hidden = requiresBreakAcknowledgement && !acknowledged;
  truckIdInput.disabled = requiresBreakAcknowledgement && !acknowledged;
  destinationInput.disabled = requiresBreakAcknowledgement && !acknowledged;
}
...
function openDutyModal() {
  ...
  dutyWarningAcknowledge.checked = false;
...
async function saveDutyAction() {
  ...
  if (dutyAction === "IN") {
    if (requiresBreakAcknowledgement && !dutyWarningAcknowledge.checked) {
...
dutyWarningAcknowledge.addEventListener("change", updateDutyGateState);
